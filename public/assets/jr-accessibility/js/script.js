...
/* =====================================================================
   PART 2: FLOATING PANEL CONTROLLERS (Quick Access + Social Media)
   ===================================================================== */
(function () {
    ...
    const closeFabSheet = () => {
        if (!fabMobile) return;
        fabMobile.classList.remove('open');
        fabMobile.setAttribute('aria-expanded', 'false');
    };

    const anyPanelOpen = () => controllers.some(ctrl => ctrl.isOpen());

    const setupFloatingPanel = ({ panelId, overlaySelector, toggleAction, closeAction, bodyClass }) => {
        const panel = document.getElementById(panelId);
        if (!panel) return;

        const overlay = overlaySelector ? document.querySelector(overlaySelector) : null;
        const toggleSelector = `[data-action="${toggleAction}"]`;
        const closeSelector = `[data-action="${closeAction}"]`;
        const toggles = document.querySelectorAll(toggleSelector);
        if (!toggles.length) return;

        let open = false;

        const setOpen = (state, options = {}) => {
            const next = Boolean(state);
            if (next === open) return;

            if (next && !options.silent) {
                controllers.forEach(ctrl => {
                    if (ctrl !== api) ctrl.setOpen(false, { silent: true });
                });
            }
            ...
            document.body.classList.toggle('floating-panel-open', anyPanelOpen());
            if (bodyClass) {
        ...
        const api = {
            setOpen: (state, options) => setOpen(state, options),
            isOpen: () => open,
        };
        ...
        document.addEventListener('click', (event) => {
            ...
            if (toggle) { handleToggle(event); return; }

            const closer = event.target.closest(closeSelector);
            if (closer) { handleClose(event); return; }
    ...
    setupFloatingPanel({
        panelId: 'quickAccessPanel',
        overlaySelector: '.quick-access-overlay',
        toggleAction: 'toggleQuickAccess',
        closeAction: 'closeQuickAccess',
        bodyClass: 'quick-access-open',
    });

    setupFloatingPanel({
        panelId: 'socialPanel',
        overlaySelector: '.social-panel-overlay',
        toggleAction: 'toggleSocialPanel',
        closeAction: 'closeSocialPanel',
        bodyClass: 'social-panel-open',
    });
})();
...
/* =====================================================================
   PART 4: FEEDBACK FORM (Rating, Captcha, Submission)
   ===================================================================== */
(function () {
    const ratingSlider = document.getElementById('fbRating');
    const emoji = document.querySelector('.feedback-rating__emoji');
    const emojiFace = document.querySelector('[data-role="ratingEmoji"]');
    const ratingLabel = document.querySelector('[data-role="ratingLabel"]');
    const feedbackForm = document.getElementById('feedbackForm');
    const feedbackModal = document.getElementById('feedbackModal');
    const feedbackAlert = document.getElementById('fbAlert');
    const fbSend = document.getElementById('fbSend');
    const fbCaptchaQuestion = document.getElementById('fbCaptchaQuestion');
    const fbCaptchaInput = document.getElementById('fbCaptcha');
    const fbCaptchaReload = document.getElementById('fbCaptchaReload');
    ...
    if (emoji) {
        emoji.addEventListener('animationend', () => { emoji.classList.remove('is-animating'); });
    }

    let updateRating = null;

    if (ratingSlider && emoji && emojiFace && ratingLabel) {
        const ratingMap = {
            1: { icon: '😡', label: 'Very dissatisfied' },
            2: { icon: '😕', label: 'Not great' },
            3: { icon: '😊', label: 'Satisfied' },
            4: { icon: '😄', label: 'Happy' },
            5: { icon: '🤩', label: 'Delighted' },
        };

        const updateSliderGradient = (value) => {
            const percent = ((value - 1) / 4) * 100;
            ratingSlider.style.setProperty('--feedback-rating-progress', `${percent}%`);
        };

        updateRating = (value) => {
            const ratingValue = ratingMap[value] || ratingMap[3];
            emoji.dataset.face = String(value);
            emojiFace.textContent = ratingValue.icon;
            ratingLabel.textContent = ratingValue.label;
            ratingSlider.setAttribute('aria-valuenow', String(value));
            ratingSlider.setAttribute('aria-valuetext', ratingValue.label);
            updateSliderGradient(value);

            emoji.classList.remove('is-animating');
            void emoji.offsetWidth;
            emoji.classList.add('is-animating');
        };

        ratingSlider.addEventListener('input', (event) => { updateRating(Number(event.target.value)); });
        ratingSlider.addEventListener('change', (event) => { updateRating(Number(event.target.value)); });

        updateRating(Number(ratingSlider.value || 3));
    }

    const updateCsrfToken = (response) => {
        if (!csrfHeader || !csrfTokenMeta || !response) return;
        const nextToken = response.headers.get(csrfHeader);
        if (nextToken) { csrfTokenMeta.setAttribute('content', nextToken); }
    };
    ...
    const loadCaptcha = async () => {
        if (!fbCaptchaQuestion) return;
        fbCaptchaQuestion.textContent = '...';
        try {
            const response = await fetch(`${siteUrl}feedback/captcha`, { method: 'GET' });
            updateCsrfToken(response);
            const data = await response.json();
            fbCaptchaQuestion.textContent = data.question || '--';
        } catch (error) {
            fbCaptchaQuestion.textContent = '--';
        }
    };

    const enforceDigits = (event) => {
        const target = event.target;
        const sanitized = target.value.replace(/[^0-9]/g, '');
        if (target.value !== sanitized) { target.value = sanitized; }
    };

    if (fbCaptchaInput) { fbCaptchaInput.addEventListener('input', enforceDigits); }

    if (fbCaptchaReload) {
        fbCaptchaReload.addEventListener('click', (event) => {
            event.preventDefault();
            loadCaptcha();
        });
    }
    ...
    if (feedbackModal) {
        feedbackModal.addEventListener('show.bs.modal', () => {
            clearAlert();
            if (fbCaptchaInput) {
                fbCaptchaInput.value = '';
                fbCaptchaInput.classList.remove('is-invalid');
            }
            loadCaptcha();
        });
    }
    ...
    if (feedbackForm) {
        feedbackForm.addEventListener('submit', async (event) => {
            ...
            const rating = Number(ratingSlider?.value || 3);
            const captchaAnswer = fbCaptchaInput?.value.trim() || '';
            document.getElementById('fbMsg')?.classList.remove('is-invalid');

            if (!message) {
                setAlert('Please share your feedback message.', 'danger');
                document.getElementById('fbMsg')?.classList.add('is-invalid');
                return;
            }
            document.getElementById('fbMsg')?.classList.remove('is-invalid');

            if (!captchaAnswer) {
                setAlert('Please solve the captcha before submitting.', 'danger');
                fbCaptchaInput?.classList.add('is-invalid');
                return;
            }
            fbCaptchaInput?.classList.remove('is-invalid');

            if (fbSend) { fbSend.disabled = true; }
            ...
            try {
                ...
                if (csrfHeader && csrfTokenMeta) {
                    headers[csrfHeader] = csrfTokenMeta.content;
                }

                const response = await fetch(`${siteUrl}feedback`, {
                    method: 'POST',
                    headers,
                ...
                if (!response.ok) {
                    const errorMessage = result?.message || result?.messages?.error || 'Failed to send feedback.';
                    throw new Error(errorMessage);
                }

                setAlert(result?.message || 'Thanks! Your feedback has been sent.', 'success');
                feedbackForm.reset();
                document.getElementById('fbMsg')?.classList.remove('is-invalid');
                fbCaptchaInput?.classList.remove('is-invalid');
                if (ratingSlider) {
                ...
                setAlert(error.message || 'Failed to send feedback. Please try again.', 'danger');
            } finally {
